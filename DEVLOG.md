...
最大的教训就是，一开始定的调调还是不能太高

如果让我再从头开始做这个项目的话，我还是会先定好大概的架构，这个过程本身没问题，不过我应该会做更多的调研，尤其是正式开发的场景下，像这种比较常见的痛点，其实应该一开始就想到肯定有人做了相似的工作，这样在相似项目的引导下应该会更快有一个整体的把握

定好架构之后一个很重要的事情就是，我觉得应该要把分出来的不同模块分别实现，跑通多个demo这样子

像我这次就比较着急的，定好架构之后直接全项目generate了，就导致整个前后端各种part上来就联调，然后报各种奇怪的错误，头都大了 X(

然后就是关于目前的实现，比较遗憾的点是没有来得及实现多模态这块，这个在devlog里面其实是有构思大概要怎么做的，如果再多几天时间的话，下一步应该就是要把图/视频片段模态这块给做出来

当然还有一个比较鸡肋的就是，我设计的这个数据结构其实也没有很好的实现，这个跟分词器有关系，我并没有足够的去找一个很好的分词器，就导致实际上产出的paragraphs很冗长，分割效果并不好，接下来继续开发也会着重在这一块做好优化处理
